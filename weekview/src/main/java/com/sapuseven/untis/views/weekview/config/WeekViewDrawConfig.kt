...
class WeekViewDrawConfig(context: Context) {
	...
	private fun buildDefaultDateTimeInterpreter(context: Context): DateTimeInterpreter {
		return object : DateTimeInterpreter {
			...
			override fun interpretDate(date: DateTime) = date.toString(datePattern, Locale.getDefault()).uppercase()

			override fun interpretSecondaryDate(date: DateTime) = date.toString(secondaryDatePattern, Locale.getDefault()).uppercase()
